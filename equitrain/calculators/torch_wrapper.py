...
class TorchWrapperPredictor:
    ...
    def predict(
        ...
        graph_list = [
            self.atoms_to_graphs.convert(atoms.copy()) for atoms in atoms_list
        ]
        loader = self.geom_loader.DataLoader(
            dataset=graph_list,
            batch_size=min(self.batch_size, len(graph_list)),
            shuffle=False,
            drop_last=False,
            pin_memory=False,
            num_workers=0,
        ...
        with self.torch.no_grad():
            for batch in loader:
                ...
                energies.extend(
                    [
                        float(x)
                        for x in energy_tensor.reshape(-1).detach().cpu().tolist()
                    ]
                )
                ...
                if require_forces:
                    ...
                    for i in range(len(ptr) - 1):
                        force_list.append(
                            np.asarray(forces_cpu[ptr[i] : ptr[i + 1]], dtype=float)
                        )
