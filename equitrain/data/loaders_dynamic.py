...
class DynamicGraphCollater:
    ...
    def __call__(self, batch):
        ...
        for item in batch:
            ...
            if self.max_edges is not None and self.drop and item.num_edges > self.max_edges:
                # Drop item which has more nodes than allowed
                continue

            if self.max_nodes is not None and self.drop and item.num_nodes > self.max_nodes:
            ...
            if current_batch:
                ...
                if self.max_nodes is not None and current_node_sum + item.num_nodes > self.max_nodes:
                ...
                if self.max_edges is not None and current_edge_sum + item.num_edges > self.max_edges:
                    # Maximum number of edges reached
                    dynamic_batches.append(self.collate_fn(current_batch))
                    current_batch    = []
                    current_node_sum = 0
...
class DynamicGraphLoader(torch_geometric.loader.DataLoader):
    def __init__(self, *args, max_nodes = None, max_edges = None, drop = False, **kwargs):
        ...
        self.collate_fn = DynamicGraphCollater(self.collate_fn, max_nodes = max_nodes, max_edges = max_edges, drop = drop)
