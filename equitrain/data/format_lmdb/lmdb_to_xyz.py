...
def convert_to_ase_object(data):
    ...
    if len(cell) == 6:  # For triclinic or hexagonal systems
        atoms = Atoms(
            numbers=atomic_numbers, positions=positions, cell=cell[:3], pbc=data['pbc']
        )
        atoms.set_cell(cell)  # Setting 6-component cell (lengths + angles)
    elif len(cell) == 3:  # For orthorhombic systems
        atoms = Atoms(
            numbers=atomic_numbers, positions=positions, cell=cell, pbc=data['pbc']
        )
    ...
    with open(output_xyz, 'w') as xyz_file:
        for i, data in enumerate(dataset):
            ...
            for symbol, pos, force in zip(
                ...
                atoms.get_positions(),
                atoms.arrays.get('forces', np.zeros_like(atoms.get_positions())),
            ):
...
#     convert_aselmdb_to_xyz(
#         '/home/cmadaria/equitrain/equitrain/tests/data/lmdb/val.aselmdb',
