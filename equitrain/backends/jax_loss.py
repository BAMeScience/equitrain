...
class LossComponent:
    ...
    def update(self, value: float, count: float):
        if count <= 0.0:
            return
        if self.count == 0.0:
...
class JaxLossCollection:
    def __init__(self):
        self.components = {
            'total': LossComponent(),
            'energy': LossComponent(),
            'forces': LossComponent(),
            'stress': LossComponent(),
        }

    def update_from_metrics(self, metrics: dict[str, tuple[float, float]]):
        for key, (value, count) in metrics.items():
            if key in self.components:
                self.components[key].update(value, count)

    def mean(self) -> float:
        return self.components['total'].value if self.components['total'].count else float('nan')
...
class LossSettings:
    ...
    def from_args(cls, args):
        ...
        return cls(
            ...
            loss_type=_canonical_loss_type(_get('loss_type', 'huber')),
            loss_type_energy=_canonical_loss_type(
                _get('loss_type_energy', None) or _get('loss_type', 'huber')
            ),
            loss_type_forces=_canonical_loss_type(
                _get('loss_type_forces', None) or _get('loss_type', 'huber')
            ),
            loss_type_stress=_canonical_loss_type(
                _get('loss_type_stress', None) or _get('loss_type', 'huber')
            ),
...
def build_loss_fn(apply_fn, settings: LossSettings):
    if settings.energy_weight <= 0.0 and settings.forces_weight <= 0.0 and settings.stress_weight <= 0.0:
